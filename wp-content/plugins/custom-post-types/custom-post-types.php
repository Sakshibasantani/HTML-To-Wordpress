...
if (!class_exists('totalpress_custom_post_types')) {
    class totalpress_custom_post_types
    {
                ...
                if (is_array($supports) && (($admin_only == 'true' && current_user_can('administrator')) || ($admin_only !== 'true' && current_user_can('edit_posts')))) {
                    foreach ($supports as $content) {
                        ...
                        if(strpos($content, '/') !== false){
                            $content_type = explode('/', $content)[0];
        ...
        public function notices()
        {
            if (!is_admin()) return;
            if (
                cpt_is_current_screen($this->cpt_ui_name) ||
                cpt_is_current_screen($this->cpt_ui_name . '_tax') ||
                cpt_is_current_screen($this->cpt_ui_name . '_field') ||
                cpt_is_current_screen($this->cpt_ui_name . '_template')
                ) {
                $buttons = [
                    [
            ...
            (new CPT\Notice)->add(
                'version_4_notice',
                "<strong><u>Version 4.x.x will be released on May 1, 2023. This version contains important changes.</u></strong><br><br>
                Thanks to the support of the <strong>thousands of users who use this amazing plugin</strong>, we have worked to improve and make this the best plugin for customizing WordPress.<br>
                The new version makes the plugin a real framework, it will make it easy to extend WordPress with post types, taxonomies, field groups, templates and <strong>settings pages</strong> directly from the WordPress dashboard.<br>
                The same customizations will be possible via PHP code using specific hooks, also allowing you to add <strong>administrative notices</strong>.<br>
                We are sure that this will make your experience with the plugin much better.<br>
                As with all major releases that include major changes to avoid problems or data loss, <strong>we encourage all users using versions prior to 4.x.x to backup the database or turn off automatic updates</strong>.<br><br>
                <strong><u>We are not responsible for any problems or loss of data.</u></strong><br><br>
                The previous FREE version is always available, to download previous versions visit the <strong>PREVIOUS VERSIONS</strong> section on the Advanced View page.<br>
                After the release, for all users who report bugs or malfunctions we will offer <strong>special discount coupons</strong>.<br><br>
            ...
            (new CPT\Notice)->add(
                'beta_4_notice',
                "<strong><u>Version 4.x.x BETA is ready.</u></strong><br><br>
                You can download and install the new version (in staging or local environment or new websites).<br>
                Here are 100 reasons to ask for your help ☺︎<br>
                The <strong>first 50 users who send bug reports or suggestions</strong> will receive the <strong><u>PRO version free for lifetime</u></strong>.<br>
                The <strong>following 50 users who send bug reports or suggestions</strong> will receive a <strong><u>lifetime discount code (70% value) for the PRO version</u></strong>.<br>
                All users who send bug reports or valid suggestions will be able to try the PRO version free for 1 month.<br><br>
                Thank you all for the support ❤",
                'success',
                false,
                -1,
                [
                    [
                        'link' => 'https://99zw.short.gy/cpt-4-beta',
                        'label' => __('Download BETA version', 'custom-post-types'),
                        'target' => '_blank',
                        'cta' => true
                    ],
                    [
                        'link' => 'https://totalpress.org/support?utm_source=wp-dashboard&utm_medium=installed-plugin&utm_campaign=custom-post-types-beta',
                        'label' => __('Send bug reports or suggestions', 'custom-post-types'),
                        'target' => '_blank'
                    ]
                ]
            );
...
<?php
        ...
        {
            $installedVersion = get_option('custom_post_types_version', null);
            ...
            update_option('custom_post_types_version', $currentVersion);
            update_option('custom_post_types_installation_time', time());
        }
    }
